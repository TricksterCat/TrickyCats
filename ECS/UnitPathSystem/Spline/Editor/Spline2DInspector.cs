...
public class Spline2DInspector : Editor {
	...
	public override void OnInspectorGUI () {
		...
		EditorGUI.BeginChangeCheck();
		bool showArrows = EditorGUILayout.Toggle("Show Arrows", spline.ShowArrows);
		if (EditorGUI.EndChangeCheck()) {
			Undo.RecordObject(spline, "Toggle Show Arrows");
			spline.ShowArrows = showArrows;
		}
    ...
    private void DrawPoints() {
		...
		if (spline.ShowArrows)
		{
			Handles.color = Color.yellow;
			var len = spline.Length;
			int index = 0;
			for (float dist = 0.0f; dist <= len; dist += spline.distanceMarker, index++)
			{
				if(index % 3 == 0)
					continue;
			
				float t = spline.DistanceToLinearT(dist);
				var nextT = spline.DistanceToLinearT((dist + spline.distanceMarker) % len);
			
				Vector3 p1 = spline.InterpolateWorldSpace(t);
				Vector3 p2 = spline.InterpolateWorldSpace(nextT);
			
				float size = math.clamp(HandleUtility.GetHandleSize(p1), 1, 2);
				Handles.Button(p1, Quaternion.LookRotation(p2 - p1), size * handleSize * 15, size * handleSize * 15, Handles.ArrowHandleCap);
			}
		}
